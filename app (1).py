...
def semantic_search_papers(groq_client, embedding_model, user_question: str, max_results: int = 20):
    ...
    for paper in candidates:
        paper_embedding = embed_text(embedding_model, paper["summary"])
...
class PaperVectorStore:
    ...
    def __init__(self, chunks, embedding_model):
        ...
        vectors = np.array([embed_text(embedding_model, c["text"]) for c in chunks], dtype="float32")
...
def render_process_button(groq_client, embedding_model, gemini_client):
    count = selected_count()
    if count == 0:
        return

    st.markdown("---")
    process_clicked = st.button(f"🚀 Process {count} selected paper(s)", type="primary")
    ...
    for turn in st.session_state.chat_history:
        ...
        st.markdown(
            ...
            <div class="chat-row">
                ...
                <div class="bubble-sources">Sources: {html.escape(sources_str) if sources_str else "none"}</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
def main():
    ...
    render_results()
    render_process_button(groq_client, embedding_model, gemini_client)
    render_summary()
