...
def _build_qa_messages(vector_store, reranker_model, question: str, conversation_history, top_k: int = 5):
    """Shared prep for both streaming and non-streaming answer generation:
    retrieval (Stage H) + message assembly. Returns (messages, sources)."""
...
def answer_question(groq_client, vector_store, reranker_model, question: str, conversation_history, top_k: int = 5):
    messages, sources = _build_qa_messages(vector_store, reranker_model, question, conversation_history, top_k)
...
def answer_question_stream(groq_client, vector_store, reranker_model, question: str, conversation_history, top_k: int = 5):
    """Same pipeline as answer_question, but returns a (token_generator, sources)
    pair so the UI can render the reply as it's generated, token by token."""
    messages, sources = _build_qa_messages(vector_store, reranker_model, question, conversation_history, top_k)
...
def answer_bubble_html(answer_text: str, sources, show_sources: bool = True) -> str:
    """Builds the HTML for one answer bubble (+ sources line), reused for both
    stored history turns and the live streaming placeholder."""
    if not show_sources:
        sources_line = ""
    else:
        # Paper titles are always English; wrap them LTR so they don't get
        # mirrored inside an RTL (Arabic) sources line.
        source_bits = [f'<span class="keep-ltr">"{html.escape(title)}", page {page}</span>' for title, page in sources]
    ...
    return f"""
    <div class="chat-row">
        <div class="bubble-a">{html.escape(answer_text).replace(chr(10), '<br>')}
        {sources_line}
        </div>
    </div>
    """
...
def render_chat(groq_client, reranker_model):
    ...
    for turn in st.session_state.chat_history:
        ...
        st.markdown(answer_bubble_html(turn["answer"], turn["sources"], show_sources=not no_match), unsafe_allow_html=True)

    question = st.chat_input(t("chat_placeholder"))
    if question:
        st.markdown(f'<div class="chat-row"><div class="bubble-q">{html.escape(question)}</div></div>', unsafe_allow_html=True)

        placeholder = st.empty()
        full_answer = ""
        try:
            token_generator, sources = answer_question_stream(
                groq_client,
            ...
            for token in token_generator:
                full_answer += token
                # Sources are only known to apply once we can tell this isn't a
                # "no match" refusal, so they're withheld until streaming ends.
                placeholder.markdown(answer_bubble_html(full_answer, sources, show_sources=False), unsafe_allow_html=True)

            no_match = is_no_answer(full_answer)
            placeholder.markdown(answer_bubble_html(full_answer, sources, show_sources=not no_match), unsafe_allow_html=True)

            st.session_state.chat_history.append(
                {"question": question, "answer": full_answer.strip(), "sources": sources}
            )
        except Exception as e:
